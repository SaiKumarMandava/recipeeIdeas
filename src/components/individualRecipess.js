  ...
  return (
    <div>
        ...
        <div className="flex flex-col justify-center">
          ...
          <div className="bg-white border-l-4 shadow-sm border-deep-purple-accent-400 h-24">
              <div className=" p-5 border border-l-0 rounded-r h-24">
                ...
                <a href={data[0]?.strSource} target='/blank' className="text-sm text-gray-900">
               Click Here
                </a>
              </div>
            </div>
      ...
      <div className="overflow-y-auto py-4 ">
        <p className='font-satoshi py-3 font-regular text-[20px] text-gray-600'>Ingredients Used</p>
  <table className="min-w-full divide-y divide-gray-200">
    <thead className="bg-gray-50">
      <tr>
        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Ingredient Name</th>
    ...
    {data && data.map((item) => (
  <tbody key={item.idMeal}  className="bg-white divide-y divide-gray-200 ">
    {[...Array(20)].map((_, index) => {
      ...
      return ingredient ? (
        <tr key={index}>
          <td className="px-6 py-4 whitespace-nowrap">{ingredient || "NA"}</td>
          <td className="px-6 py-4 whitespace-nowrap">{measure || "NA"}</td>
        </tr>
      ) : null;
    })}
  </tbody>
      ...
      <details className=' border-2 rounded-md px-6 py-4'>
        <summary className="text-base text-gray-600 md:text-lg py-4 font-satoshi font-bold">Instructions</summary>
    ...
    <div className='py-4 '>
    <p className='font-satoshi py-3 font-regular text-[20px] text-gray-600'>Video Reference</p>
    {
      data[0]?.strYoutube?(
        <iframe 
        className="w-full flex justify-center lg:h-[500px] rounded-md aspect-video md:aspect-square" 
      
        src={data[0]?.strYoutube ? data[0].strYoutube.replace("watch?v=", "embed/") : ""}
    ...
    <p className='font-satoshi py-3 font-regular text-[20px] text-gray-600'>Video Reference Not Available</p>
